...
DRUG_QUERIES = {
    ...
    "AG013": "AG013",
    "GC4419（Avasopasem）": "avasopasem",
    "SGX942（Dusquetide）": "dusquetide",
    "EC-18": "EC-18",
    "RRx-001": "RRx-001",
    "瑞巴派特": "rebamipide",
    "美洲大蠊提取物": "Periplaneta americana",
    "Hangeshashinto（TJ-14）": "Hangeshashinto",
    "沙格司亭（GM-CSF）含漱液": "sargramostim",
}
...
HEADERS = {
    "User-Agent": "OM-Drug-Pipeline/1.0 (automated weekly update)",
    "Accept": "application/json",
}
CONDITION = "oral mucositis"   # 适应症限定
...
def main():
    ...
    for drug in drugs:
        ...
        if phase_cn:
            # 保留“已终止/未达终点/拒批”等人工判断的修饰
            current = drug.get("phase", "")
            if "终止" in current or "拒批" in current or "未达" in current:
